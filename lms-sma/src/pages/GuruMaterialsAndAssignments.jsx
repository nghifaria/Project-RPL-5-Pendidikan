  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="flex gap-4 justify-center mb-6">
        <button
          className={`btn px-6 ${tab === 'materi' ? 'btn-primary' : 'btn-outline'}`}
          onClick={() => {
            setTab('materi')
            setTitle('')
            setDesc('')
        ...
        <button
          className={`btn px-6 ${tab === 'tugas' ? 'btn-primary' : 'btn-outline'}`}
          onClick={() => {
            setTab('tugas')
            setTitle('')
            setDesc('')
      ...
      {tab === 'materi' && (
        <div className="bg-white border border-gray-300 rounded-2xl shadow-xl p-8 mb-8">
          <form className="mb-6" onSubmit={handleUploadMateri}>
            <h3 className="font-semibold mb-2 text-black font-sans">Upload Materi</h3>
            <input
              type="text"
              placeholder="Judul Materi"
              className="input input-bordered w-full mb-2 bg-white text-black placeholder-gray-400 border border-gray-400 font-sans"
              value={title}
            ...
            <input
              ...
              className="file-input file-input-bordered w-full mb-2 bg-white text-black border border-gray-400 font-sans"
              style={{
                backgroundColor: '#fff',
                color: '#111',
                borderColor: '#a3a3a3',
            ...
            />
            <button type="submit" className="btn btn-primary w-full rounded-xl text-base font-bold tracking-wide py-3 mt-2 font-sans">
              <span className="text-lg font-bold">+</span> Upload Materi
            </button>
            {status && (
      ...
      {tab === 'tugas' && (
        <div className="bg-white border border-gray-200 rounded-xl shadow p-6 mb-8">
          <form className="mb-6" onSubmit={handleCreateAssignment}>
            <h3 className="font-semibold mb-2 text-gray-500">Buat Tugas Baru</h3>
            <input
              type="text"
              placeholder="Judul Tugas"
              className="input input-bordered w-full mb-2 bg-white text-gray-900 placeholder-gray-400 text-lg px-5 py-3 rounded-xl"
              value={title}
              onChange={e => setTitle(e.target.value)}
            />
            <textarea
              placeholder="Deskripsi (opsional)"
              className="textarea textarea-bordered w-full mb-2 bg-white text-gray-900 placeholder-gray-400 text-lg px-5 py-3 rounded-xl"
              value={desc}
            ...
            <input
              ...
              className="file-input file-input-bordered w-full mb-2 bg-white text-gray-900 rounded-xl"
              style={{ backgroundColor: '#fff', color: '#1f2937', borderColor: '#e5e7eb' }}
            />
            <button type="submit" className="btn btn-primary w-full rounded-xl text-base font-bold tracking-wide py-3 mt-2">
              Buat Tugas
            </button>
            {statusAssign && (
              <div className={`mt-2 text-sm px-3 py-2 rounded ${
                statusAssign.startsWith('✅') ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'
              }`}>
                {statusAssign}
              </div>
            )}
          ...
          {assignments.length === 0 ? (
            ...
            <ul className="space-y-2">
              {assignments.map(asgn => (
                <li key={asgn.id} className="bg-violet-50 border border-violet-100 rounded p-3 flex flex-col">
                  <div>
